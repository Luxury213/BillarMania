...
export default function GameScreen() {
  ...
  const handleAimEnd = (releaseX: number, releaseY: number) => {
    ...
    const dx = start.x - releaseX;
    const dy = start.y - releaseY;
  ...
  const gesture = Gesture.Pan()
    .onBegin((e) => {
      runOnJS(handleAimStart)(e.x, e.y);
    })
    .onUpdate((e) => {
      runOnJS(handleAimMove)(e.x, e.y);
    })
    .onEnd((e) => {
      runOnJS(handleAimEnd)(e.x, e.y);
    });
