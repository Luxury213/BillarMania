...
function getRackPositions() {
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    for (let c = 0; c < numBalls; c++) {
      ...
      positions.push({ id: ballId, x, y: startY + c * BALL_R * 1.9 });
      ballId++;
    }
  }
  return positions;
...
export default function GameScreen({ onSalir }: { onSalir?: () => void }) {
  ...
  const resolveShot = () => {
    ...
    setDisplayState({
      ...
      activeBooster: sd.activeBooster,
    });
  ...
  const jsFire = useCallback((dx: number, dy: number) => {
    ...
    Matter.Body.setVelocity(cue, { x: dir.x * power * 20, y: dir.y * power * 20 });
  ...
  const gesture = Gesture.Pan()
    ...
    .onStart(e => {
      'worklet';
      if (svStep.value === 0) {
        svStep.value = 1; svOriginX.value = e.x; svOriginY.value = e.y;
        runOnJS(jsStartAim)(e.x, e.y);
      } else if (svStep.value === 1) {
        svStep.value = 2; svOriginX.value = e.x; svOriginY.value = e.y;
    ...
    .onEnd(e => {
      ...
      if (svStep.value === 1) {
        ...
        svStep.value = 0; runOnJS(jsFire)(dx, dy);
      }
    });
      ...
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <View style={styles.hud}>
          <View style={styles.hudBlock}>
            ...
            <Text style={[styles.hudVal, { color: C.accent }]}>{displayState.round}/10</Text>
          </View>
          ...
          <View style={styles.hudBlock}>
            ...
            <Text style={[styles.hudVal, { color: C.gold }]}>🪙 {displayState.coins}</Text>
          </View>
          <View style={styles.hudSep} />
          <View style={styles.hudBlock}>
              ...
              {showAim && lineEnd && (
                <>
                  <Line
                    ...
                    color={aimLineColor()}
                    strokeWidth={step === 2 ? 2 + power * 2 : (displayState.activeBooster === 'precision' ? 2.5 : 2)}
                  />
...
const styles = StyleSheet.create({
  ...
  canvas:          { flex: 1 },
  hud:             { position: 'absolute', top: 0, left: 0, right: 0, height: HUD_H, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around', paddingHorizontal: 8, backgroundColor: 'rgba(6,9,16,0.96)', borderBottomWidth: 1, borderBottomColor: C.primary + '33', zIndex: 10 },
  ...
  debugTitle:      { color: C.accent, fontSize: 12, fontWeight: 'bold', textAlign: 'center', marginBottom: 6, letterSpacing: 2 },
  debugInfo:       { color: C.gold, fontSize: 11, fontFamily: 'monospace', marginBottom: 4, textAlign: 'center' },
